...
const Create = (): React.ReactNode => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (selectedIndex === -1) {
      // create new post
      createPost(title, code); // 使用 `createPost` 函數建立新貼文
      ...
      // edit old post
      updatePost(selectedIndex, title, code); // 使用 `updatePost` 函數編輯舊貼文
      toast({
  ...
  return (
    <div className="flex h-auto overflow-y-scroll">
      <div className="flex w-xs flex-col gap-0 p-2">
        ...
        {user &&
          getPostIndicesByUserId(user._id).map((postIndex) => {  // Hint 3.2.1: 使用 `getPostIndicesByUserId`
            const post = getPostByIndex(postIndex);  // Hint 3.2.2: 使用 `getPostByIndex`
            if (post === null) return <></>;
            return (
              <span
                key={post._id}
