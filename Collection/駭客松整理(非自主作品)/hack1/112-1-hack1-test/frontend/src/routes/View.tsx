...
const View = (): React.ReactNode => {
  ...
  const handlePrevClick = useCallback(() => {
    setSelectedIndex(prevIndex => Math.max(prevIndex - 1, 0)); // Ensure we don't go into negative indices
  }, []);
  /* End (3/3) TODO 2.2 */
  ...
  const hasUpvoted = post && user ? post.upvotes.includes(user._id) : false;
  const hasDownvoted = post && user ? post.downvotes.includes(user._id) : false;
  ...
  useEffect(() => {
    ...
    // Add `handleKeyPress` function as event listener to keyboard input event
