...
const updatePost = async (index: number, title: string, content: string) => {
  ...
  try {
    ...
    if (!postToUpdate) {
      console.error("No post found at the specified index.");
      return;
    }

    const payload = { title, content };
    await PostService.update(postToUpdate._id, payload); // update post in DB

    /* Hint 3.3.2: Use React hook to update frontend */
    setPosts(prevPosts => {
      if (!prevPosts) return null;  // 如果 prevPosts 是 null，直接返回 null
